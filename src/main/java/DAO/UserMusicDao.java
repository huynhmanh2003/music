...
public class UserMusicDao {
    ...
    public static String findMusic(int id) {
 Connection con = null;
    PreparedStatement stmt = null;
    ConnectDatabase db = new ConnectDatabase();
    String musics = "";
    ...
    try {
        con = db.connect();
        String query = "select MusicID from UserMusic WHERE UserId = ?";
        stmt = con.prepareStatement(query);
        stmt.setInt(1, id);
        rs = stmt.executeQuery();

        boolean isFirst = true; // Đánh dấu xem dòng hiện tại có phải là dòng đầu tiên không
        while (rs.next()) {
            if (isFirst) {
                musics = "" + rs.getInt(1);
                isFirst = false;
            } else {
                musics = musics + "-" + rs.getInt(1);
            }
        }
        return musics;
    } catch (SQLException ex) {
...
//        System.out.println(u1.getUserMusic());
        System.out.println(UserMusicDao.findMusic(1));
    }
